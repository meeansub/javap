...
public class MainFrame extends JFrame{
	...
	public JPanel gameView(Game game) {
		...
		betBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(userBet > game.getUser().getUserCoin()||totalUserBet>game.getUser().getUserCoin()) {//배팅 건 갯수가 더크다면
	...
	public JPanel aiDieCheck(int aiDie) {
		...
		else
		{
			...
			{
				//                  list.add("추가 배팅 3회초과");
				//                  list.add(String.format("game.getAi().number()는 %d", game.getAi().number(game)));
